...
class PageVrise(Page):
   ...
   def __init__(self, *args, **kwargs):
       ...
       self.title_vrise = tk.PhotoImage(file="/Users/jean/Documents/Dev/SmartSolarDesign/databases/Images/Titles/VriseTitle.png")
   ...
   def show_entries(self,job_dict,user_pref,inv_dict):
       ...
       lbl_title1 = ttk.Label(self, image = self.title_vrise)
       lbl_title1.grid(row = constants.ROW_TITLE_1_PGVRISE,pady=constants.SPACING_FROM_TITLE_PGVRISE,columnspan=7)
       lbl_empty_space=ttk.Label(self, text="",font='Helvetica 14 bold').grid(row = constants.ROW_TITLE_1_PGVRISE+1,columnspan=7,pady=constants.SPACING_FROM_TITLE_PGVRISE)

       #1st line
       self.butt_lock_names.grid(row=constants.ROW_NAME_ENT_PGVRISE, column=5)
       if self.var_lock_name == 0:#Condition so it only happens once (nad not when prev/next is hit)
           self.lock_names()
       #2nd line
       lbl_phases = ttk.Label(self, text="Inv phases:").grid(row=constants.ROW_PHASE_ENT_PGVRISE, column=1, sticky="e")
       self.butt_lock_inv_phase.grid(row=constants.ROW_PHASE_ENT_PGVRISE, column=5)
       if self.var_lock_inv_phases == 0:#Condition so it only happens once (nad not when prev/next is hit)
           self.lock_inv_phases()
       #3rd line
       lbl_cond1 = ttk.Label(self, text="Conductor:").grid(row=constants.ROW_CONDUCTOR_ENT_PGVRISE, column=1,pady=constants.ROW_SPACING_TABLE_PADY_PGVRISE, sticky="e")
       self.butt_lock_wire_type.grid(row=constants.ROW_CONDUCTOR_ENT_PGVRISE, column=5)
       if self.var_lock_wire_type == 0:#Condition so it only happens once (nad not when prev/next is hit)
           self.lock_wire_type()
       #4th line
       lbl_imax = ttk.Label(self, text="Imax/phase:").grid(row=constants.ROW_IMAX_ENT_PGVRISE, column=1, sticky="e",pady=constants.ROW_SPACING_TABLE_PADY_PGVRISE)
       self.butt_lock_imax_phase.grid(row=constants.ROW_IMAX_ENT_PGVRISE, column=5)
       ...
       self.combobox_msb_wire_size.grid(row=constants.ROW_SIZE_ENT_PGVRISE, column=4)
       ...
       #6th line
       lbl_len1 = ttk.Label(self, text="Lenght [m]:").grid(row=constants.ROW_LENGHT_ENT_PGVRISE, column=1, sticky="e",pady=constants.ROW_SPACING_TABLE_PADY_PGVRISE)
       self.ent_serv_len.grid(row=constants.ROW_LENGHT_ENT_PGVRISE, column=2,pady=2)
       self.ent_cons_len.grid(row=constants.ROW_LENGHT_ENT_PGVRISE, column=3)
       self.ent_msb_len.grid(row=constants.ROW_LENGHT_ENT_PGVRISE, column=4)

       #Calculation part (the rest is in calculate_Vrise)
       lbl_imax = ttk.Label(self, text="Am/%Vd :").grid(row=constants.ROW_AM_ENT_PGVRISE, column=1, sticky="e",pady=constants.ROW_SPACING_TABLE_PADY_PGVRISE)
       lbl_Vdrop_prct = ttk.Label(self, text="Vdrop % :").grid(row=constants.ROW_VDROP_ENT_PGVRISE, column=1, sticky="e",pady=constants.ROW_SPACING_TABLE_PADY_PGVRISE)
       lbl_Vdrop_prct_tot = ttk.Label(self, text="Total %:").grid(row=constants.ROW_TOTAL_PRC_ENT_PGVRISE, column=1, sticky="e",pady=constants.ROW_SPACING_TABLE_PADY_PGVRISE)
       self.butt_calculate.grid(row=constants.ROW_CALC_BUTT_PGVRISE,columnspan=7,pady=20)

       #Notes part
       lbl_notes = ttk.Label(self, text="Vrise Notes:").grid(row=constants.ROW_NOTES_ENT_PGVRISE, column=1, sticky="e",pady=10)
       self.ent_notes.grid(row=constants.ROW_NOTES_ENT_PGVRISE,column = 2,columnspan=3,pady=10)

       self.grid_rowconfigure(0, weight=1)
       self.grid_rowconfigure(constants.ROW_NOTES_ENT_PGVRISE+1, weight=1)
   ...
   def calculate_Vrise(self):
       ...
       lbl_l7_left.grid(row=constants.ROW_AM_ENT_PGVRISE, column=2)
       lbl_l7_mid = ttk.Label(self, text=Am2)
       lbl_l7_mid.grid(row=constants.ROW_AM_ENT_PGVRISE, column=3)
       lbl_l7_right = ttk.Label(self, text=Am3)
       lbl_l7_right.grid(row=constants.ROW_AM_ENT_PGVRISE, column=4)
   ...
   def lock_names(self,*args):
       if self.var_lock_name == 0:#Only happens once on instance
            ...
            self.lbl_col1_name.grid(row=constants.ROW_NAME_ENT_PGVRISE, column=2)
            self.lbl_col2_name.grid(row=constants.ROW_NAME_ENT_PGVRISE, column=3)
            self.lbl_col3_name.grid(row=constants.ROW_NAME_ENT_PGVRISE, column=4)
            ...
            self.ent_col1_name.grid(row=constants.ROW_NAME_ENT_PGVRISE, column=2)
            self.ent_col2_name.grid(row=constants.ROW_NAME_ENT_PGVRISE, column=3)
            self.ent_col3_name.grid(row=constants.ROW_NAME_ENT_PGVRISE, column=4)
   ...
   def lock_inv_phases(self,*args):
       ...
       if self.var_lock_inv_phases == 0:#Only happens once on instance
            ...
            self.lbl_new_phase_inv_1.grid(row=constants.ROW_PHASE_ENT_PGVRISE, column=2)
            self.lbl_new_phase_inv_2.grid(row=constants.ROW_PHASE_ENT_PGVRISE, column=3)
            self.lbl_new_phase_inv_3.grid(row=constants.ROW_PHASE_ENT_PGVRISE, column=4)
            ...
            self.ent_new_phase_inv_1.grid(row=constants.ROW_PHASE_ENT_PGVRISE, column=2)
            self.ent_new_phase_inv_2.grid(row=constants.ROW_PHASE_ENT_PGVRISE, column=3)
            self.ent_new_phase_inv_3.grid(row=constants.ROW_PHASE_ENT_PGVRISE, column=4)
   ...
   def lock_imax_phase(self,*args):
       ...
       if self.var_lock_imax_phase == 0:#Only happens once on instance
            ...
            self.lbl_new_imax_phase1.grid(row=constants.ROW_IMAX_ENT_PGVRISE, column=2)
            self.lbl_new_imax_phase2.grid(row=constants.ROW_IMAX_ENT_PGVRISE, column=3)
            self.lbl_new_imax_phase3.grid(row=constants.ROW_IMAX_ENT_PGVRISE, column=4)
            ...
            self.ent_new_imax_phase1.grid(row=constants.ROW_IMAX_ENT_PGVRISE, column=2)
            self.ent_new_imax_phase2.grid(row=constants.ROW_IMAX_ENT_PGVRISE, column=3)
            self.ent_new_imax_phase3.grid(row=constants.ROW_IMAX_ENT_PGVRISE, column=4)
   ...
   def lock_wire_type(self,*args):
       if self.var_lock_wire_type == 0:#Only happens once on instance
            ...
            self.lbl_new_wire_type_1.grid(row=constants.ROW_CONDUCTOR_ENT_PGVRISE, column=2)
            self.lbl_new_wire_type_2.grid(row=constants.ROW_CONDUCTOR_ENT_PGVRISE, column=3)
            self.lbl_new_wire_type_3.grid(row=constants.ROW_CONDUCTOR_ENT_PGVRISE, column=4)
            ...
            self.combobox_serv_wire_type.grid(row=constants.ROW_CONDUCTOR_ENT_PGVRISE, column=2)
            ...
            self.combobox_cons_wire_type.grid(row=constants.ROW_CONDUCTOR_ENT_PGVRISE, column=3)
            ...
            self.combobox_msb_wire_type.grid(row=constants.ROW_CONDUCTOR_ENT_PGVRISE, column=4)
